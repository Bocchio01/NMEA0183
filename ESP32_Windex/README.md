...
The easiest way to set up the ESP32 boards I think is to use the Arduino IDE.
...
> In case you want to upload the `Sender` code, you also need to deconnect the Windex sensor from the ESP board (remove the cable from the `RX` pin of the ESP board).

## Load the library into the sketch

To use the `NMEA0183` library, you need to include it in your sketch.
You can do this by creating a `src` folder in the root of your project and copying all the `*.c` and `*.h` files from the `NMEA0183` library into the `src` folder.
...
So far I've marked with a black dot over the connection port the ESP board that is currently used as receiver.
...
| ----------- | ----------------- |
| Receiver    | 08:D1:F9:27:F3:A0 |
| Transmitter | To be retrived    |
